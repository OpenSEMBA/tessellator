...
private:
    ...
    std::set<Element> getElementsConvertedToLines(const Cell cell, std::map<Cell, std::vector<const Element*>> cellElemMap);
    void splitLinesWithNeighborTriangle(const GroupId groupIndex, std::set<Element> elementsConvertedToLines, Group& meshGroup, const Cell cell, std::map<Cell, std::vector<const Element*>> cellElemMap, std::vector<std::set<ElementId>> toRemove);
};
