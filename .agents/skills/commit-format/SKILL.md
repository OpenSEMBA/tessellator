---
name: commit-format
description: Provides this repository's required commit-message format, categories, and examples. Use when creating, reviewing, or proposing git commit messages for Tessellator.
license: MIT
metadata:
  author: Elemwave
  version: "1.0"
...
3. **Write repository commit message**: Use exact format `Tessellator | <type> | #<Issue> | <Description>`. `Tessellator` is literal. Include GitHub issue number, such as `#866`.
Allowed types
   - `feat`: new feature
   - `fix`: bug fix
...
Write description in imperative mood, concise, and ideally 72 characters or fewer.
   - Good: `Tessellator | chore | #866 | Use relative path for STEP file in TULIP JSON export`
   - Good: `Tessellator | feat | #880 | Add validation for solver parameter ranges`
...
4. **Apply commit rules**:
   ...
   - Preserve capitalisation for `Tessellator`, categories, proper nouns, and identifiers.
