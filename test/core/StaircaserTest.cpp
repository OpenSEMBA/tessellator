...
TEST_F(StaircaserTest, transformTriangleWithEquidistantEdges)
{
    ...
    //  *---┼------/--*  | |      ->       *---┼----------*    ║             *--┼-┼---------*   |      ->    *----╫----------*   ⎹
    //  |   |y   ╱    | ⎹  |               ⎸   |     <-   |    ║             | |  |y    \   |   |            |    ║ y   ->   |   ⎹
    //  |   *--╱------┼-┼--*               ⎸{1.66->4}═════╪{0.5|1.33->1}     | ⎸  *------\--┼---*            {1.5|2.33->2}═══╪{2.66->4}
    //  |  / ╱        ⎹⎹  /                ⎸ /⫽\\\\\\\\\\⎹\\\⫽              ||  /        \ ⎹   /            |  ⩘⫽\\\\\\\\\\⎹\\\⫽
    //  | /╱          ⎹⎸ /                 ⎸⩗⫽\\\\\\\\\\\|\\⫽⩘             ⎹| /          \ ⎸ /             ⎹ /⫽\\\\\\\\\\\\|\⫽ /
    //  |⫽            ║/                  ⎹⫽\\\\\\\\\\\\\|⫽ /              ⎹⎸/            \⎸/               ⎸⫽\\\\\\\\\\\\\|⫽ ⩗
    //  2═════════════0 x              {0->3}═════════════2 x                1══════════════0 x              1═══════════════0 x
    //                                          ->                                                                  ->
    // 
...
TEST_F(StaircaserTest, selectiveStaircaserWithEmptySetOfCells)
{
    ...
    Relatives expectedRelatives(mesh.coordinates);

    Elements expectedElements(mesh.groups[0].elements);
...
TEST_F(StaircaserTest, staircaseMoreThanOneCell)
{
    // *-------------*-------------*          *---------------*-------------(4->3) 
    // |             |             |          |               |               ║ 
    // |             |             |          |               |               ║ 
...
TEST_F(StaircaserTest, verifyOrderInSelectiveStaircaser)
{
    // *-------------*-------------*        (4->3)══════════(3->2)-------------*
    // |     4───────3-_           |          |               | ‾-_           | 
    // |             |  ‾-_        |          |               |    ‾-_        | 
...
TEST_F(StaircaserTest, selectiveStaircaserFillingGapsInFrontier_Split)
{
    //         4══════════_-=2               5═══════════════3
    //        ⫽|      _-‾‾  /║              ⫽\\\\\\\\\\\\\⫽/║     
    //       ⫽ |  __-‾     /⎹║             ⫽\\\\\\\\\\\\\⫽//║       
...
TEST_F(StaircaserTest, selectiveStaircaserFillingGapsInFrontier_Insert)
{
    //         4══════════_-=2                5══════════════3
    //        ⫽|      _-‾‾  /║              ⫽\\\\\\\\\\\\\⫽⎹⎸     
    //       ⫽ |  __-‾     /⎹║             ⫽\\\\\\\\\\\\\⫽/⎹⎸       
...
TEST_F(StaircaserTest, selectiveStaircaser_SplitLinesWithNeighborTriangle)
{
    ...
    std::set<Cell> cellSet;
    cellSet.insert(Cell({1, 0, 0}));
    
    Mesh mesh;
    mesh.grid = GridTools::buildCartesianGrid(lowerCoordinateValue, upperCoordinateValue, numberOfCells);
    mesh.coordinates = {
        Relative({ 0.4, 2.0, 1.0 }), // 0
        Relative({ 1.0, 1.4, 1.0 }), // 1
        Relative({ 1.0, 2.0, 1.0 }), // 2
        Relative({ 1.4, 1.0, 1.0 }), // 3
        Relative({ 2.0, 1.0, 1.0 }), // 4
        Relative({ 2.0, 2.0, 1.0 }), // 5
        Relative({ 2.0, 0.0, 1.0 }), // 6
    };
    ...
    mesh.groups[0].elements = {
        Element({0, 1, 2}, Element::Type::Surface),
        Element({1, 3, 2}, Element::Type::Surface),
        Element({3, 4, 2}, Element::Type::Surface),
        Element({2, 4, 5}, Element::Type::Surface),
        Element({3, 6, 4}, Element::Type::Surface),
    };

    Relatives expectedRelatives = {
        Relative({ 1.0, 1.0, 1.0 }), // (3->0) 
        Relative({ 2.0, 1.0, 1.0 }), // (4->1) 
        Relative({ 2.0, 0.0, 1.0 }), // (6->2) 
        Relative({ 0.4, 2.0, 1.0 }), // (0->3) 
        Relative({ 1.0, 1.4, 1.0 }), // (1->4)
        Relative({ 1.0, 2.0, 1.0 }), // (2->5)
        Relative({ 2.0, 2.0, 1.0 }), // (5->6) 
    };
...
TEST_F(StaircaserTest, selectiveStaircaser_staircase_insert)
{
    ...
    auto resultMesh = Staircaser{ mesh }.getSelectiveMesh(cellSet, Staircaser::GapsFillingType::Split);
